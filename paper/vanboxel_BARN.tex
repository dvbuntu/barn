...
Based on the current implementation, we see that BARN is significantly slower than all of the other methods tested.  Note, however, that the equivalently sized neural network is not nearly as slow.  Part of this is that BARN has to train networks many times (i.e. the burn in period plus the sampling period, several hundred iterations in this study).  But part of this cost is also constantly loading/unloading BARN's NNs into memory (as stored by scikit-learn).
...
\begin{table}[htb]
\centering
\caption{Mean computation time (and standard deviation) over 13 runs of various methods on different datasets.  Smallest mean time in bold. \dvb{TODO}}
\begin{tabular}{lrrr}
Dataset   &     BARN &    Big NN &      BART \\ \hline
boston    & 4.64 ( 0.447)&  8.11 ( 4.958)&  \textbf{4.10} ( 0.556)\\
concrete  & 7.32 ( 0.537)& 12.68 ( 7.990)&  \textbf{6.82} ( 0.436)\\
